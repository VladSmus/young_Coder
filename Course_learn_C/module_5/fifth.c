...
bool isPrime(int n) {
    // Проверяем случаи малых чисел вручную
    if (n <= 1)
        return false;

    // Для чётных чисел сразу возвращаем False кроме случая 2
    if ((n % 2 == 0 && n != 2))
        return false;

    // Ищем делители среди нечётных чисел от 3 до sqrt(n)
    int limit = n / 2 + 1;   // Верхняя граница для перебора
    for (int i = 3; i * i <= n; i += 2) { // Перебираем нечётные числа начиная с 3
        if (n % i == 0)
            return false;
    }
    return true;
...
int main() {
    ...
    // Если число простое выводим 1 иначе 0
    printf(isPrime(N) ? "1\n" : "0\n");
